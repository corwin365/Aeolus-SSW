...
%smooth?
Settings.SmoothSize = [1,1]; %time units, height units - both depend on gridding choices
...
for iYear=1:1:numel(Years)
  ...
  Var2(Var2 < 0) = Var2(Var2 < 0)./2;  
  [c,h] = contour(t2,Data.Settings.HeightScale,Var2',[  5:5:60],'edgecolor',[1,1,1].*0.4,'linewi',0.5,'linestyle', '-');  clabel(c,h);
  [c,h] = contour(t2,Data.Settings.HeightScale,Var2',[-60:2.5:-2.5],'edgecolor',[1,1,1].*0.4,'linewi',0.5,'linestyle', ':');  clabel(c,h);
  [c,h] = contour(t2,Data.Settings.HeightScale,Var2',     [0,0],'edgecolor',[1,1,1].*0.4,'linewi',1.5,'linestyle', '-');  clabel(c,h);
...
%%
%colourbar
colormap(flipud(cbrewer('div','RdBu',numel(-60:5:60)-1)))
cb =colorbar('position',[0.05 0.33 0.02 0.33]);
caxis([-1,1].*30);
ticks = -30:10:30; labels = ticks; labels(labels < 0) = labels(labels <0)./2;

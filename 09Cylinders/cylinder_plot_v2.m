...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%plot components of the figure which combines a cylinder and wings. 
%
%this is a monstrosity of hardcoding, but it works - you can read it if you
%like...
%
%
%Corwin Wright, c.wright@bath.ac.uk, 2021/03/12
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Settings.Day = datenum(2021,1,17);
...
for iWing = 3:1:numel(Settings.Wings)

  cla
  set(gcf,'color','w','position',[680 258 803 420])
  ...
  set(gca,'fontsize',14)
  if iWing == 1;  set(gca,'yaxislocation','left'); end
  ...
  if iWing == 2; text(Settings.MinLat,max(Settings.YRange).*1.05,[num2str(Settings.Wings(iWing)),'E'],'fontsize',18,'fontweight','bold','horizontalalignment','right'); end
...
% figure
clf
set(gcf,'color','w','position',[255,1,587,788])
subplot = @(m,n,p) subtightplot (m,n,p,0.04,0.05,0.1);

for iWing = 1:1:numel(Settings.Wings)+1

  subplot(numel(Settings.Wings)+1,1,iWing)
  ...
  if iWing ~= numel(Settings.Wings)+1;
    lonidx = closest(squeeze(xo(1,:,1)),Settings.Wings(iWing));
    U  = squeeze(U(:,lonidx,:))'; clear lonidx
  ...
  if iWing == numel(Settings.Wings)+1;
    ...
    for itick=1:1:numel(xticks); xlabels{itick} = [num2str(xticks(itick)),'N'];end
    set(gca,'xtick',xticks,'xticklabel',xlabels)
  else set(gca,'xtick',xticks,'xticklabel',{}); end
  yticks = 0:6:44;
  ylabels = cell(numel(yticks),1);
  for itick=1:1:numel(yticks); ylabels{itick} = [num2str(yticks(itick)),'km'];end
  set(gca,'ytick',yticks,'yticklabel',ylabels)  
  ...
  %labelling
  if iWing ~=numel(Settings.Wings)+1;
    text(Settings.MinLat,max(Settings.YRange).*1.05,[num2str(Settings.Wings(iWing)),'E'],'fontsize',18,'fontweight','bold','horizontalalignment','left');    
  else
    text(Settings.MinLat,max(Settings.YRange).*1.05,['Zonal Mean'],'fontsize',18,'fontweight','bold','horizontalalignment','left');
  end
  ...
  %plot some solid edges - easy edges
  plot([Settings.MinLat,80],[1,1].*min(Settings.YRange),'k-','linewi',2)
  plot([1,1,].*Settings.MinLat,Settings.YRange,'k-','linewi',2)
  plot([1,1,].*80,Settings.YRange,'k-','linewi',2)
...
export_fig(['allwings'],'-png','-m2','-a4','-transparent')
...
%we also need an unwrapped labelled version - generate this now.
set(gcf,'color','w','position',[1975,127,682,325])
axis on
xticks = 0:60:360;
xlabels = cell(numel(xticks),1);
for itick=1:1:numel(xticks); xlabels{itick} = [num2str(xticks(itick)),'E'];end
set(gca,'xtick',xticks,'xticklabel',xlabels)
...
export_fig('cylinder_wrap','-png','-m3','-a4');%,'-transparent')
